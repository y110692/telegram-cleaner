...
const elements = {
  ...
  footerStats: document.querySelector("#stats-inner"),
  openPrivacyBtn: document.getElementById("nav-privacy-btn"),
  ...
  stage: document.querySelector(".app-main"),
  ...
  messageTime: null, // Время теперь внутри messageDate
  ...
  openSettingsBtn: document.getElementById("nav-settings-btn"),
  ...
  desktopOnlyNotice: document.getElementById("desktop-only-notice"),
  statsBar: document.getElementById("stats-bar"),
  navPrivacyBtn: document.getElementById("nav-privacy-btn"),
  footerWhyTitle: document.querySelector(".footer-why-title"),
  footerToTopBtn: document.getElementById("footer-to-top-btn"),
  footerWhy: document.getElementById("footer-why"),
  footerWhyToggle: document.getElementById("footer-why-toggle"),
};
...
const STORAGE_KEYS = {
  settings: "telegram-cleaner-v2.settings.v1",
  decisionsPrefix: "telegram-cleaner-v2.decisions.v1.",
};
...
function readStoragePayload(primaryKey, fallbackKey, fallbackValue = {}) {
  const primaryRaw = localStorage.getItem(primaryKey);
  if (primaryRaw) {
    return safeJsonParse(primaryRaw, fallbackValue);
  }
  const fallbackRaw = localStorage.getItem(fallbackKey);
  if (fallbackRaw) {
    return safeJsonParse(fallbackRaw, fallbackValue);
  }
  return fallbackValue;
}

function storedSettingsPayload() {
  const stored = readStoragePayload(STORAGE_KEYS.settings, LEGACY_STORAGE_KEYS.settings, {});
  ...
  // One-time migration to v2 namespace.
  if (!localStorage.getItem(STORAGE_KEYS.settings) && localStorage.getItem(LEGACY_STORAGE_KEYS.settings)) {
    persistSettingsPayload(normalized);
  }
  return normalized;
}
...
function legacyExportStorageKey(exportKey) {
  return exportKey ? `${LEGACY_STORAGE_KEYS.decisionsPrefix}${exportKey}` : "";
}
...
function restoreExportState(exportKey) {
  const stored = readStoragePayload(
    `${STORAGE_KEYS.decisionsPrefix}${exportKey}`,
    legacyExportStorageKey(exportKey),
    {},
  );
  ...
  if (!localStorage.getItem(`${STORAGE_KEYS.decisionsPrefix}${exportKey}`) && localStorage.getItem(legacyExportStorageKey(exportKey))) {
...
async function pickExportFolder() {
  if (!isDesktopMode()) {
    throw new Error("Функционал доступен только с компьютера");
  }
  if (typeof window.showDirectoryPicker === "function") {
    try {
      ...
      console.error("Error in pickExportFolder:", error);
      throw error;
    }
    return;
  }
...
function syncStageVerticalAlignment() {
  ...
  const stageHidden = layout.classList.contains("hidden");
  if (stageHidden || state.isMobileViewport || !state.exportReady) {
    return;
  }

  const actionButtonsHeight =
    elements.actionButtons && !elements.actionButtons.classList.contains("hidden") ? elements.actionButtons.offsetHeight : 0;
  const statsHeight = elements.statsBar && !elements.statsBar.classList.contains("hidden") ? elements.statsBar.offsetHeight : 0;
  const interSectionGap = actionButtonsHeight > 0 && statsHeight > 0 ? 48 : actionButtonsHeight > 0 || statsHeight > 0 ? 24 : 0;
  const groupHeight = layout.offsetHeight + actionButtonsHeight + statsHeight + interSectionGap;
  const freeSpace = stage.clientHeight - groupHeight;

  if (freeSpace <= 0) {
    layout.classList.add("is-overflowing");
    return;
  }

  layout.style.paddingTop = `${Math.floor(freeSpace / 2)}px`;
}
...
function renderStats() {
  ...
  if (elements.statLeft) elements.statLeft.textContent = left;
  if (elements.statSkipped) elements.statSkipped.textContent = skipped;
  if (elements.statSaved) elements.statSaved.textContent = saved;
...
function renderUndoButton() {
  ...
  // Скрываем кнопку "Назад" если нет истории или экспорт не готов
  const canUndo = state.history.length > 0 && state.exportReady;
  elements.undoCardBtn.disabled = !canUndo;
  elements.undoCardBtn.classList.toggle("hidden", !canUndo);
}
...
function updateSettingsSaveButtonState() {
  const hasPendingTag = Boolean(elements.settingsTagInput?.value.trim());
  elements.saveSettingsTagsBtn.disabled = !hasPendingTag;
  elements.saveSettingsTagsBtn.classList.toggle("settings-save-ready", hasPendingTag);
}
...
function renderCard() {
  ...
  renderActionButtons();
  resetStageScroll();
  const isMobileViewport = state.isMobileViewport;

  if (isMobileViewport) {
    if (elements.uploadSection) elements.uploadSection.classList.add("hidden");
    if (elements.desktopOnlyNotice) elements.desktopOnlyNotice.classList.remove("hidden");
    if (elements.stageLayout) elements.stageLayout.classList.add("hidden");
    if (elements.actionButtons) elements.actionButtons.classList.add("hidden");
    if (elements.statsBar) elements.statsBar.classList.add("hidden");
    state.renderedItemId = null;
    return;
  }

  if (elements.desktopOnlyNotice) elements.desktopOnlyNotice.classList.add("hidden");
  if (!state.exportReady) {
    // Показываем секцию загрузки, скрываем основной интерфейс
  ...
  // Скрываем секцию загрузки, показываем основной интерфейс
  if (elements.uploadSection) elements.uploadSection.classList.add("hidden");
  if (elements.stageLayout) elements.stageLayout.classList.remove("hidden");
  if (elements.actionButtons) elements.actionButtons.classList.remove("hidden");
  if (elements.statsBar) elements.statsBar.classList.remove("hidden");
  ...
  elements.actionButtons?.classList.toggle("hidden", Boolean(state.savePanelItem));
  elements.card.style.transform = "";
  state.renderedItemId = item.id;
  elements.messageDate.textContent = `${item.date_display} · ${item.time_display}`;
...
function renderSavePanel() {
  ...
  if (!item) {
    ...
    elements.actionButtons?.classList.toggle("hidden", !state.exportReady || !state.currentItem);
  ...
  const orderedTags = getOrderedKnownTags();
...
function registerButtons() {
  ...
  elements.navInfoBtn?.addEventListener("click", () => {
    scrollToElementTop(elements.footerWhyTitle);
  });
  ...
  elements.footerToTopBtn?.addEventListener("click", () => {
  ...
  elements.settingsTagInput.addEventListener("input", () => {
    updateSettingsSaveButtonState();
  });
...
function initFooterWhyToggle() {
  const section = elements.footerWhy;
  const toggle = elements.footerWhyToggle;
  if (!section || !toggle) {
    return;
  }
  toggle.addEventListener("click", () => {
    ...
    toggle.setAttribute("aria-label", nextOpen ? "Свернуть преимущества" : "Раскрыть преимущества");
  });
}

function scrollToElementTop(node) {
  if (!node) {
    return;
  }
  const navHeight = document.querySelector(".top-nav")?.offsetHeight || 0;
  const targetY = window.scrollY + node.getBoundingClientRect().top - navHeight - 10;
  window.scrollTo({ top: Math.max(0, targetY), behavior: "smooth" });
}

function syncHeaderBadgeVisibility() {
  const navInner = document.querySelector(".nav-inner");
  const navBrand = document.querySelector(".nav-brand");
  const navActions = document.querySelector(".nav-actions");
  const subtitle = document.querySelector(".nav-subtitle");
  if (!navInner || !navBrand || !navActions || !subtitle) {
    return;
  }

  subtitle.classList.remove("is-collapsed");

  const innerWidth = navInner.clientWidth;
  const brandWidth = navBrand.offsetWidth;
  const actionsWidth = navActions.offsetWidth;
  const navInnerStyles = window.getComputedStyle(navInner);
  const horizontalPadding =
    (Number.parseFloat(navInnerStyles.paddingLeft) || 0) +
    (Number.parseFloat(navInnerStyles.paddingRight) || 0);
  const reserveGap = 16; // breathing room between brand and actions
  const required = brandWidth + actionsWidth + reserveGap;
  const available = Math.max(0, innerWidth - horizontalPadding);

  if (required > available) {
    subtitle.classList.add("is-collapsed");
  }
}

function initScrollReveal() {
  const targets = Array.from(document.querySelectorAll(".footer-why-title, .feature, .plus-item"));
  if (!targets.length) {
    return;
  }
...
async function init() {
  state.isMobileViewport = window.matchMedia("(max-width: 720px)").matches;
  if (state.isMobileViewport) {
    // На мобильных отключаем desktop-only точки входа
    ...
    if (elements.uploadSection) elements.uploadSection.classList.add("hidden");
    if (elements.desktopOnlyNotice) elements.desktopOnlyNotice.classList.remove("hidden");
    if (elements.navPrivacyBtn) elements.navPrivacyBtn.classList.add("hidden");
    if (elements.pickExportBtn) elements.pickExportBtn.classList.add("hidden");
    if (elements.emptyPickExportBtn) elements.emptyPickExportBtn.classList.add("hidden");
    if (elements.downloadOutputBtn) elements.downloadOutputBtn.classList.add("hidden");
    if (elements.pickOutputBtn) elements.pickOutputBtn.classList.add("hidden");
    if (elements.openOutputFolderBtn) elements.openOutputFolderBtn.classList.add("hidden");
    if (elements.undoCardBtn) elements.undoCardBtn.classList.add("hidden");
    if (elements.actionButtons) elements.actionButtons.classList.add("hidden");
    if (elements.statsBar) elements.statsBar.classList.add("hidden");
  }
